...
- **A worked example** — the bundled *Data Overview* dashboard.
...
python3 test/browser/seed.py     # admin user + enabled dashboard + sample collections
node test/browser/verify.cjs
```

It drives headless Chrome through the gallery, the runner and the config page as both an
anonymous and an admin user, and fails on any console error or failed request. Screenshots are
written to `test/browser/screenshots/`. Configure with `GIRDER_URL`, `GIRDER_ADMIN`,
